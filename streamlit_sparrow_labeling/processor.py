class DataProcessor:
    ...
    def prepare_canvas_data(
        self, data, background_color, doc_height, doc_width, height, width
    ):
        canvas_rects = []

        for rect in data["words"]:
            canvas_rect = self.construct_canvas_rect(
                rect, doc_height, doc_width, height, width
            )
    ...
    def construct_canvas_rect(self, rect, doc_height, doc_width, height, width):
        canvas_rect = {
            ...
            "width": round((rect["rect"]["x2"] * width) / doc_width)
    ...
    def prepare_rect_data(
        self, canvas_data, initial_rects, doc_height, doc_width, height, width
    ):
        rects = []

        for i, canvas_rect in enumerate(canvas_data["objects"]):
            if i < len(initial_rects["words"]):
                rect = self.construct_rect(
                    canvas_rect,
                    initial_rects["words"][i],
                    doc_height,
                    doc_width,
                    height,
                    width,
        ...
        payload = {
            "meta": {
                "version": initial_rects["meta"]["file_name"],
                "split": initial_rects["meta"]["page_number"],
                "image_id": initial_rects["meta"]["attachment_id"],
                "image_size": {
    ...
    def construct_rect(
        self, canvas_rect, initial_rect, doc_height, doc_width, height, width
    ):
    ...
    def update_rect_data(self, rect_data, rect_index, value, label):
        rect_data["words"][rect_index]["text"] = value
